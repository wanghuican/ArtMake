...
public class ProductAction extends ActionSupport {
	 ...
	 * setter method
	 * 
	 * @param productIngList
	 *            the productIngList to set
	 */
	 ...
	 * setter method
	 * 
	 * @param productNotIngList
	 *            the productNotIngList to set
	 */
	...
	public String toProductIngList() {
		...
		List<Product> productIngList = productService.getProductByIng(pro,
	...
	public String toProductNotIngList() {
		...
		List<Product> productNotIngList = productService.getProductByNotIng(
				pro, page, Common.PAGESIZE - 1);
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("page", page);
	...
	public String toProductJoinList() {
		...
		List<Integer> pro = new ArrayList<Integer>();
		pro.add(getId());
		int count = proActionService.countProductPerson(person);
		int page = 0;
		page = WebTool.dealPage(count, page, Common.PAGESIZE, "PAGEING");
		List<Product> productIngList = proActionService.getProductListPerson(person,
				page, Common.PAGESIZE);
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("page", page);
		map.put("rows", WebTool.listToJsonStr(productIngList));
	...
	public String delProduct() throws IOException {
		...
		for (int i = 0; i < product.getImageList().size(); i++) {
			String imagepath = Common.PLAT_SRC + Common.PRODUCT_SRC + "/"
					+ product.getImageList().get(i).getImage();
	...
	public String goEditProduct() {
		if (getId() != 0) {
			Product product = productService.getProductById(getId());
			setProduct(product);
			if (product.getPerson().getPerson_id() != WebTool
					.getSessionPerson().getPerson_id()
					|| product.getState() == 1) {
				setProAction(proActionService.getFirstAction(getId()));
	...
	public String goPutProduct() {
		Product product = productService.getProductById(getId());
		setProduct(product);
		setAuth(authService.getAuthByRole_id(WebTool.getSessionPerson()
				.getRole().getRole_id()));
		return "put";
	}
	...
	public String createRecord() throws IOException {
		if (!Common.LOCK[getId()]) {
			...
			Product pro = pa.getProduct();
			pro.setPrice(price);
			productService.saveProduct(pro);
			Common.LOCK[getId()] = false;
			WebTool.alertMessage("竞拍成功", "product!goEditProduct?id="+getId());
		}else{
