...
<script type="text/javascript">
	$(document).ready(function(){
	    ...
	    $("#selectcode").val(pro[0]);
	    $("#selectname").val(pro[1]);
	    
	    <s:iterator value="roleList" id="row">
	    $("#update<s:property value='#row.role_id'/>").fancybox({
	    	'href' : 'roleManage!goEdit?id=<s:property value='#row.role_id'/>',
	    	'width' : 733,
	        'height' : 530,
	        'type' : 'iframe',
	...
	/** 模糊查询  **/
	function search(){
		$("#submitForm").attr("action", "roleManage").submit();
	}
	...
	function del(id){
		if(confirm("您确定要删除吗？")){
			$("#submitForm").attr("action", "roleManage!delRole?id=" + id).submit();			
		}
	}
	...
	function batchDel(){
		...
		//alert(allIDCheck)
		if(confirm("您确定要删除吗？")){
    		$("#submitForm").attr("action", "roleManage!ndelRole?nid="+allIDCheck).submit();
		}
	}
	
	function jumpNormalPage(page,key){
		if(jumpPage(page,key)){
			$("#submitForm").attr("action", "roleManage?PAGE="+ page).submit();
		}
	}
			...
			<div class="ui_content">
				<div class="ui_text_indent">
					<div id="box_border">
						...
						<div id="box_center">
							角色码
							<input type="text" id="selectcode" name="selectpro" class="ui_input_txt02" />					
						         角色名<input type="text" id="selectname" name="selectpro" class="ui_input_txt02" />
						</div>
						<div id="box_bottom">
				    		<input type="button" value="查询" class="ui_input_btn01" onclick="search();" /> 
							<input type="button" value="新建" id="addbtn" class="ui_input_btn01"/> 
				...
				<div class="ui_tb">
					<table class="table" cellspacing="0" cellpadding="0" width="100%" align="center" border="0">
						<tr>
							...
							<th width="19%">角色码</th>
							<th width="23%">角色名</th>
							<th width="23%">下级角色名</th>
							<th width="25%">操作</th>
						</tr>
						<s:iterator value="roleList" id="row">
							<tr>
								<td><input type="checkbox" name="IDCheck" value="<s:property value='#row.role_id'/>" class="acb" /></td>
								<td><s:property value="#row.role_code"/></td>
								<td><s:property value="#row.rolename"/></td>
								<td><s:property value="#row.frolename"/></td>
								<td>
									<s:if test="#session.auth == 100">
							    		<label style="cursor:pointer" id="update<s:property value='#row.role_id'/>">修改</label>
						    			<label style="cursor:pointer" onclick="del(<s:property value='#row.role_id'/>)">删除</label>
								    </s:if>
								</td>
							</tr>
						</s:iterator>
