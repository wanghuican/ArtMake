...
<script type="text/javascript">
	$(document).ready(function(){
	    ...
	    $("#selecttimemin").val(pro[0] == min ? "" : pro[0]);
	    $("#selecttimemax").val(pro[1] == max ? "" : pro[1]);
	    
	    <s:iterator value="keyList" id="row">
	    $("#update<s:property value='#row.key_id'/>").fancybox({
	    	'href' : 'keyManage!goEdit?id=<s:property value='#row.key_id'/>',
	    	'width' : 733,
	        'height' : 530,
	        'type' : 'iframe',
	...
	/** 模糊查询  **/
	function search(){
		$("#submitForm").attr("action", "keyManage").submit();
	}
	...
	function del(id){
		if(confirm("您确定要删除吗？")){
			$("#submitForm").attr("action", "keyManage!delKey?id=" + id).submit();			
		}
	}
	...
	function batchDel(){
		...
		//alert(allIDCheck)
		if(confirm("您确定要删除吗？")){
    		$("#submitForm").attr("action", "keyManage!ndelKey?nid="+allIDCheck).submit();
		}
	}
	
	function jumpNormalPage(page,key){
		if(jumpPage(page,key)){
			$("#submitForm").attr("action", "keyManage?PAGE="+ page).submit();
		}
	}
			...
			<div class="ui_content">
				<div class="ui_text_indent">
					<div id="box_border">
						...
						<div id="box_center">
							使用次数
							<input type="text" id="selecttimemin" name="selectpro" class="ui_input_txt02" />~			
			...
			<div class="ui_content">
				<div class="ui_tb">
					<table class="table" cellspacing="0" cellpadding="0" width="100%" align="center" border="0">
						...
						<s:iterator value="keyList" id="row">
							<tr>
								<td><input type="checkbox" name="IDCheck" value="<s:property value='#row.key_id'/>" class="acb" /></td>
								<td><s:property value="#row.keyname"/></td>
								<td><s:property value="#row.times"/></td>
								<td>
							    		<label style="cursor:pointer" id="update<s:property value='#row.key_id'/>">修改</label>
						    			<label style="cursor:pointer" onclick="del(<s:property value='#row.key_id'/>)">删除</label>
								</td>
							</tr>
						</s:iterator>
					</table>
